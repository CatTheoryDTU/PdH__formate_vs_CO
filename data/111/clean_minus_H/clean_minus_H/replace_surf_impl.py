...
from gpaw.solvation import (
    ...
    SurfaceInteraction)
#==========================================================
# Read calculation result & 'correct' subsurface
atoms=read('init_orig.traj')
surf=read('/home/cat/olsosi/data/CO2to2e/MetalHydrides/PdH/111/clean_slab/out.traj')

# Update positions of subsurface
n=0
diff=atoms[n].position-surf[n].position
#for atom in atoms:
#    if n < len(surf):
#        atom.position=surf[n].position
#    else:
#        atom.position=atom.position-diff
#    n=n+1
for sur in surf:
    atoms[n].position=sur.position+diff
    n=n+1
write('init_impl.traj',atoms)
